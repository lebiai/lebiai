# Hermes Flutter 客户端 — 进度看板

> Flutter 原生客户端(iOS/Android/macOS)+ `hermes-server`,作为 small-rust-hermes 的专业前端。
...
## M3 — 管理面板(L3) ✅
- [x] skills / memory / config / reflect / mcp REST(1:1 gui commands)
- [x] Flutter 管理面板:config(model 切换) + skills(查看/删除) + memory(置顶/删除)

## M4 — 移动增值(L4) ✅
- [x] **前置**: Hermes 加 `Image` `ContentBlock` + ImageSource(hermes-core)+ 全 match 点补分支
