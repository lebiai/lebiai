...
| 字段 | 内容 |
|------|------|
| **日期** | 2026-08-07（快照；2026-08-14 起部分过期） |
| **种类** | **H** 快照（非权威） |
| **权威** | 冲突只认 [`../../PRODUCT_PRINCIPLES.md`](../../PRODUCT_PRINCIPLES.md) **P0 v0.9**。本文不得当现行法。 |
| **已知过期** | IM 白名单已收紧（无 durable 写）；GUI 上下文已合并到 `hermes-channel`；默认语言 zh-CN；桌面有试用/授权；CLI 不是用户默认路径 |
...
| 后端 | `hermes-server`（axum，bearer token） | Flutter 后端；**GUI 子集 + WS**（含 inbox / ws-ticket；非 1:1） |
...
2. ✅ **「server 路由 1:1 GUI commands」已纠正为子集契约**：权威文档与能力矩阵已改为「GUI 全量 / server Flutter 子集」；inbox REST 与 ws-ticket 已补齐（仍缺 onboarding/微信等桌面专属）。
