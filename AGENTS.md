...
- 完整定义卡：[`PRODUCT_PRINCIPLES.md`](./PRODUCT_PRINCIPLES.md) 文首（P0 v0.5）
- 用户路径：配置 API Key → **对话**（禁用「聊天」作主词）→ 共事 / 谋划 → 批准进化候选
...
| ② 内置技能 | memory-palace、skill-creator、find-skills | 编译进 CLI，自动安装 | **否** |
...
1. **开工前：** 复制 [`docs/records/_TEMPLATE.md`](./docs/records/_TEMPLATE.md) 为 `docs/records/YYYYMMDD-slug.md`
   - 写清 **用户价值**
   - **产品经理：** 场景、路径变化、成功标准、不做什么
   - **架构师：** 根因、正确默认路径、边界、如何防复发
   - 若只能写出「临时 if / 遮错 / 碰运气」→ **停手重做方案**
...
| **其他文档** | 仅允许在 [`docs/`](./docs/)，服从 P0/P1 |

禁止另起第二套产品说明；禁止 Docker/远程优先写成默认主路径。
...
crates/hermes-cli      → 主入口（CLI 子命令 + 内置技能源）
crates/hermes-gui      → 桌面 GUI（Tauri 2，同引擎同数据）· **主交付面**
crates/hermes-server   → Flutter 后端（REST/WS，bearer token；路由 1:1 GUI commands）
...
细节：[`docs/gui-run.md`](./docs/gui-run.md)。
...
3. 对用户是否**更好用、更高效、越好用**（Do/Continuity/Care/Evolve 至少强化一环）？
...
8. 新增 md 是否放在 `docs/` 而非根目录？方向变更是否回写 `docs/work-companion-solution.md`？
9. **产品经理视角**是否写清（场景/路径/成功标准）？
10. **架构师视角**是否写清（根因/默认路径）？是否在避免修修补补？
...
17. **文案：** 中文主词是否为「对话」而非「聊天」？是否无锚假装记得、无交付强迫打磨？
...
CLI 是主入口；GUI / Flutter（经 `hermes-server`）/ 微信 / 飞书 / Telegram 是同等 surface，
共享同一引擎与同一份数据，**不是**平行产品。禁止某一入口独占引擎逻辑。
产品身份协议的**唯一源码**：`crates/hermes-core/src/companion.rs`。

*对齐 PRODUCT_PRINCIPLES **v0.5** / DEVELOPMENT_RULES **v0.1** · 2026-08-07*
