...
mod tests {
    ...
    fn compiled_profile_replaces_memory_sections() {
        ...
        assert!(
            !s.contains("Pinned memories"),
            "pinned section should be skipped"
        );
        assert!(
        ...
        assert!(
            !s.contains("mem_p"),
            "individual memory IDs should not appear"
        );
    ...
    fn palace_index_replaces_all_memory_sections() {
        ...
        assert!(
            ...
            "compiled profile should be skipped"
        );
        assert!(
            !s.contains("Pinned memories"),
            "pinned section should be skipped"
        ...
        assert!(
            !s.contains("Active memory index"),
            "index section should be skipped"
        );

        let t = sources.build_turn_system("how do I handle errors?");
        assert!(
            !t.contains("Relevant memories"),
            "per-turn memories should be skipped when palace active"
        );
    }
